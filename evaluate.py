...
def solve(model: RefinementUT, puzzles: np.ndarray, solutions: np.ndarray,
    ...
    for i in range(0, n, batch_size):
        ...
        hit = ((pred == sol) | ~mine).all(dim=1)
        solved[i:i + batch_size] = hit.cpu().numpy()
        finished[i:i + batch_size] = out["finished"].cpu().numpy()
        passes[i:i + batch_size] = (out["stop_pass"].float() + 1).cpu().numpy()
    return {"solved": solved, "finished": finished, "passes": passes}


def _band_table(name: str, bands, key, solved_by_budget: dict, budgets: list[int]) -> str:
    lines = [f"\n  by {name}"]
    head = "  " + f"{'band':>10}" + f"{'n':>7}" + "".join(f"{'b'+str(b):>9}" for b in budgets)
    lines.append(head)
    for lo, hi in bands:
        sel = (key >= lo) & (key <= hi)
        n = int(sel.sum())
        ...
        row = f"  {label:>10}{n:>7}"
        row += "".join(f"{solved_by_budget[b][sel].mean():>9.3f}" for b in budgets)
        lines.append(row)
    return "\n".join(lines)
